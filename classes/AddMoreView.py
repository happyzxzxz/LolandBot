    ...
    async def one_more(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if "vk.com" not in self.search:
            ...
            if len(self.vk_search_results) > 1:
                ...
                await interaction.message.edit(content=f"Added {self.vk_search_results[0].title} into the queue", view=AddMoreView(ctx=self.ctx, search=self.search, vk_search_results=self.vk_search_results))
        if not vc.current:
